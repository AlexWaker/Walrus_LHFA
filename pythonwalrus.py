...
# Std lib imports
import os
import subprocess #创建进程，可以用来运行命令行
import json
import tempfile
import base64
...
try:
    ...
    store_json_command = f"""{{ "config" : "{PATH_TO_WALRUS_CONFIG}",
        "command" : {{ "store" :
        {{ "file" : "{file_path}", "epochs" : 2  }}}}
    }}"""
    result = subprocess.run(
        [PATH_TO_WALRUS, "json"],
        text=True,
        capture_output=True,
        input=store_json_command,    #这一大块相当于执行了walrus json命令，也就是直接在命令行中执行，其返回值就是命令行执行命令之后显示的一大串结果，result就是显示的结果
    )
